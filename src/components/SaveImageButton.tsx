...
function SaveImageButton({ label, filename, canvasRef,  disabled = false } : SaveImageButtonProps) {
  ...
  return (
    <div className="flex gap-1" data-tip="Allow multiple download if you want to save more than one image">
       <a
        ref={anchorRef}
        className={`btn-custom btn btn-primary text-xl ${disabled ? "btn-disabled" : "" }`}
        onClick={ () => saveImage()}
      >
        {label}
      </a>
      <div className="dropdown dropdown-end">
        <div tabIndex={0} role="button" className="text-info cursor-pointer">
          <svg tabIndex={0} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" className="w-8 h-8 stroke-current"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
        </div>
